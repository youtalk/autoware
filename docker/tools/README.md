...
| `1`       | at least one admission rejection (`MAJOR` / `MINOR` / `TOPIC` mismatch, `NO_PROVIDER`, or a QoS pivot violation) — deploy blocked                                                                                 |
...
`test/run_self_test.sh` builds a dedicated `ADMIT_TOOL_IMAGE` containing `manifest_admit` (with the fixture spec manifest baked in for the QoS-pivot case), builds a matrix of fixture images from the manifests under `test/fixtures/manifests/`, and asserts `deploy_check.sh`'s exit code on the compose sets under `test/fixtures/compose/`:
