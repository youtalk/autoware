...
| `compose.qos-pivot-consumer.yaml` | a version-compatible pairing where the consumer requires `transient_local` durability against the fixture spec manifest's `reliable` / `volatile` pivot                                                | `1` (QOS_PIVOT_CONSUMER)                                           |
