...
graph TB
    subgraph "MRMの発動条件"
        NORMAL[正常動作状態<br/>(default_ad_api)]
        DEGRADE[性能劣化検知<br/>(system_error_monitor)]
        FAILURE[システム故障検知<br/>(emergency_handler)]
        EMERGENCY[緊急事態発生<br/>(hazard_status_converter)]
    end
    
    subgraph "MRMの種類"
        COMFORTABLE[快適停止<br/>Comfortable Stop<br/>(mrm_comfortable_stop_operator)]
        EMERGENCY_STOP[緊急停止<br/>Emergency Stop<br/>(mrm_emergency_stop_operator)]
        PULLOVER[路肩退避<br/>Pull Over<br/>(mrm_pull_over_operator)]
        PULLOUT[退避場所へ移動<br/>Pull Out<br/>(mrm_pull_out_operator)]
    end
...
graph TD
    subgraph "監視層"
        HB_MON[ハートビート監視<br/>(component_state_monitor)]
        DIAG_MON[診断監視<br/>(diagnostic_aggregator)]
        PERF_MON[性能監視<br/>(system_monitor)]
        ENV_MON[環境監視<br/>(environment_monitor)]
    end
    
    subgraph "判断層"
        FAIL_DETECT[故障検知器<br/>(failure_detector)]
        RISK_ASSESS[リスク評価器<br/>(risk_assessor)]
        MRM_SELECT[MRM選択器<br/>(mrm_selector)]
    end
    
    subgraph "実行層"
        MRM_BEHAVIOR[MRM行動計画<br/>(mrm_behavior_planner)]
        MRM_MOTION[MRM運動計画<br/>(mrm_motion_planner)]
        MRM_CONTROL[MRM制御<br/>(mrm_controller)]
    end
    
    subgraph "車両制御層"
        VEHICLE_CMD[車両コマンド<br/>(vehicle_cmd_gate)]
        HAZARD_CTRL[ハザード制御<br/>(hazard_light_controller)]
        DOOR_CTRL[ドア制御<br/>(door_controller)]
        COMM_CTRL[通信制御<br/>(remote_cmd_converter)]
    end
...
flowchart TD
    subgraph "入力監視"
        SENSOR_CHECK[センサー状態チェック<br/>(sensing_driver)]
        LOCALIZATION_CHECK[自己位置推定チェック<br/>(localization_health_checker)]
        PERCEPTION_CHECK[認識機能チェック<br/>(perception_health_checker)]
        PLANNING_CHECK[計画機能チェック<br/>(planning_health_checker)]
        CONTROL_CHECK[制御機能チェック<br/>(control_health_checker)]
    end
...
```mermaid
flowchart TD
    START[快適停止開始<br/>(mrm_comfortable_stop_operator)] --> CHECK_LANE{現在車線で<br/>停止可能?<br/>(path_safety_checker)}
    
    CHECK_LANE -->|Yes| DECEL_PLAN[減速計画生成<br/>・目標減速度: -1.0 m/s²<br/>・ジャーク制限: 0.5 m/s³<br/>(trajectory_planner)]
    CHECK_LANE -->|No| FIND_SAFE[安全な停止位置探索<br/>(goal_planner)]
    ...
    subgraph "停止後動作"
        HAZARD_ON[ハザードランプ点灯<br/>(turn_signal_decider)]
        PARK_BRAKE[パーキングブレーキ<br/>(shift_decider)]
        SHIFT_P[シフトP<br/>(shift_decider)]
        DOOR_UNLOCK[ドア解錠<br/>(door_controller)]
        NOTIFY[管制センター通知<br/>(autoware_state_panel)]
    end
...
```mermaid
flowchart TD
    TRIGGER[緊急停止トリガー<br/>(mrm_emergency_stop_operator)] --> IMMEDIATE[即座の制動開始<br/>・最大減速度: -6.0 m/s²<br/>・ABS作動許容<br/>(emergency_brake_controller)]
...
```mermaid
flowchart TD
    INIT[路肩退避開始<br/>(mrm_pull_over_operator)] --> SCAN[退避場所スキャン<br/>(pull_over_planner)]
    
    subgraph "退避場所探索"
        SHOULDER[路肩検出<br/>・幅2.5m以上<br/>・障害物なし<br/>(lanelet2_extension)]
        PARKING[駐車場検索<br/>・500m以内<br/>・アクセス可能<br/>(goal_planner)]
        EMERGENCY_BAY[非常駐車帯<br/>・1km以内<br/>(map_loader)]
    end
...
```mermaid
flowchart TD
    FAILURE_DETECTED[故障検知<br/>(system_error_monitor)] --> ASSESS{故障評価<br/>(mrm_handler)}
...
```mermaid
sequenceDiagram
    participant MON as 監視システム<br/>(system_monitor)
    participant MRM as MRM管理器<br/>(mrm_handler)
    participant PLAN as 計画システム<br/>(planning)
    participant CTRL as 制御システム<br/>(control)
    participant VEH as 車両<br/>(vehicle_interface)
    participant CENTER as 管制センター<br/>(web_controller)
...
graph TB
    subgraph "通常動作時機能"
        FULL_PERCEPTION[完全認識機能<br/>(perception)]
        FULL_PLANNING[完全計画機能<br/>(planning)]
        FULL_CONTROL[完全制御機能<br/>(control)]
        COMFORT_FEATURES[快適機能<br/>(comfort_features)]
    end
    
    subgraph "MRM動作時機能"
        LIMITED_PERCEPTION[限定認識<br/>・前方のみ<br/>・最小範囲<br/>(mrm_perception)]
        SIMPLE_PLANNING[単純計画<br/>・停止のみ<br/>・固定経路<br/>(mrm_planner)]
        SAFE_CONTROL[安全制御<br/>・保守的<br/>・冗長系<br/>(mrm_controller)]
        EMERGENCY_ONLY[緊急機能のみ<br/>(mrm_emergency)]
    end
